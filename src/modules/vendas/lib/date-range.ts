...
export type VendasSearchParams = {
  ...
  vendedorId?: string;
  corretorId?: string;
  /** Nº do pedido (dígitos) ou nome do cliente (texto) — um único campo. */
  busca?: string;
};
...
/** Intervalo a partir de atalho (datas no fuso local do servidor). */
...
/** `where.createdAt` para Prisma ou undefined se não filtrar por data. */
export function createdAtWhereFromVendasParams(
  p: VendasSearchParams,
